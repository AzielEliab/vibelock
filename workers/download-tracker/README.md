...
UI / `vibelock/ui.py`). Paste notes, fill feature fields, or upload
WAV, MP3, MP4, WebM, or a still. The browser decodes the file. The
Worker does not. Results show score, verdict, reason codes, per-check
metrics, and which channels fired (physics / linguistics / vibration /
related) — software output, not a raw JSON dump. Physics + A/V deepfake
detection is a **risk assessment**, not a lie detector, not an accuracy
rate, and not courtroom proof. Hosted is not a live mic. `/v1` does not
increment downloads.
...
Import the OpenAPI spec or connect the MCP catalog. There are no per-crawler install packages.

| Method | Path | Notes |
|--------|------|-------|
| GET | `/v1/health` | Liveness |
| GET | `/v1/skill` | Live SKILL.md |
| POST | `/v1/analyze` | Advisory multi-signal score (features / limited PCM / visual / pitch / A/V / linguistics). Does not decode MP4/MP3 bytes. |
| POST | `/v1/detect` | Same engine; deepfake-oriented body; `channels` cites physics, linguistics, vibration, related |
