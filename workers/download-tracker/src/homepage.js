...
const BANNER =
  "Physics + A/V deepfake detection is a risk assessment, not a lie detector and not courtroom proof. Hosted is not a live microphone. The Worker does not decode MP4/MP3 bytes — this page asks the browser to decode common audio/video, then calls POST /v1/analyze. Full local decode stays on 127.0.0.1 via vibelock detect when ffmpeg is installed. Channels without evidence stay insufficient. The score is a risk index, not an accuracy rate.";
...
const CLIENT_JS = [
  ...
  "      syllable_peak_ratio: ['linguistics', 'syllable_peak_ratio'], interval_cv: ['linguistics', 'interval_cv'],",
  "      n_intervals: ['linguistics', 'n_intervals'], n_pauses: ['linguistics', 'n_pauses'],",
  "      pause_cv: ['linguistics', 'pause_cv'], transition_median: ['linguistics', 'transition_median'],",
  "      coherence: ['vibration', 'coherence'], transfer_residual: ['vibration', 'transfer_residual']",
  "    };",
  "    var lines = t.split(/\\n|;/);",
  "    for (var i = 0; i < lines.length; i++) {",
  "      var line = lines[i].trim();",
  "      if (!line || line.charAt(0) === '#') continue;",
  "      var m = line.match(/^(?:(features|visual|video|pitch|av|linguistics|vibration)\\.)?([a-z0-9_]+)\\s*[:=]\\s*(-?[0-9.eE+-]+)\\s*$/i);",
  "      if (!m) continue;",
  ...
  "    if (hasKeys(buckets.linguistics)) out.linguistics = buckets.linguistics;",
  "    if (hasKeys(buckets.vibration)) out.vibration = buckets.vibration;",
  "    return out;",
  "  }",
  "",
  ...
  "    var d = img.data, n = img.width * img.height, s = 0, i;",
  "    for (i = 0; i < n; i++) {",
  "      var o = i * 4;",
  ...
  "  }",
  "",
  "  function centerHist(img) {",
  "    var h = [0, 0, 0, 0, 0, 0, 0, 0];",
  "    var w = img.width, ht = img.height, d = img.data;",
  "    var x0 = Math.floor(w * 0.25), x1 = Math.floor(w * 0.75);",
  "    var y0 = Math.floor(ht * 0.25), y1 = Math.floor(ht * 0.75);",
  "    var n = 0, y, x;",
  "    for (y = y0; y < y1; y++) {",
  "      for (x = x0; x < x1; x++) {",
  "        var p = (y * w + x) * 4;",
  "        var yv = 0.2126 * d[p] + 0.7152 * d[p + 1] + 0.0722 * d[p + 2];",
  "        h[Math.min(7, Math.floor(yv / 32))]++;",
  "        n++;",
  "      }",
  "    }",
  "    for (var i = 0; i < 8; i++) h[i] = n ? h[i] / n : 0;",
  "    return h;",
  "  }",
  "",
  "  function histL1(a, b) {",
  "    var s = 0, i;",
  "    for (i = 0; i < 8; i++) s += Math.abs(a[i] - b[i]);",
  "    return s;",
  "  }",
  "",
  ...
  "      }",
  "      video.onerror = function () { fail('This browser could not decode that video. Use vibelock detect locally, or export WAV / PNG.'); };",
  "      video.onloadeddata = function () {",
  "        var dur = video.duration;",
  "        if (!isFinite(dur) || dur <= 0) dur = 1;",
  "        var times = [];",
  "        var n = 6, i;",
  "        for (i = 0; i < n; i++) times.push(Math.min(Math.max(0, dur - 0.05), Math.max(0, dur * (i + 0.5) / n)));",
  ...
  "          catch (e) { fail('Could not seek that video.'); }",
  "        }",
  "        video.onseeked = function () {",
  "          var w = video.videoWidth || 96;",
  "          var h = video.videoHeight || 96;",
  "          var scale = Math.min(1, 96 / Math.max(w, h));",
  ...
  "  }",
  "",
  "  function metricsFromFrames(frames) {",
  "    if (!frames.length) return null;",
  "    var mid = frames[Math.floor(frames.length / 2)];",
  "    var visual = visualFromImageData(mid);",
  "    var flicker = 0, identity = 0, i;",
  "    for (i = 1; i < frames.length; i++) {",
  "      flicker = Math.max(flicker, Math.abs(meanLuma(frames[i]) - meanLuma(frames[i - 1])) / 255);",
  "      identity = Math.max(identity, histL1(centerHist(frames[i]), centerHist(frames[i - 1])));",
  "    }",
  "    return { visual: visual, video: { flicker: flicker, identity_jump: identity } };",
  "  }",
  "",
  ...
  "      var isVideo = type.indexOf('video/') === 0 || /\\.(mp4|m4v|webm|mov|mkv|avi)$/.test(name);",
  "      var isAudio = type.indexOf('audio/') === 0 || /\\.(wav|wave|flac|mp3|ogg|oga|m4a|aac)$/.test(name);",
  "      if (isImage) {",
  "        var url = URL.createObjectURL(file);",
  "        var img = new Image();",
  "        img.onload = function () {",
  ...
  "      }",
  "      if (isVideo) {",
  "        var audioP = new Promise(function (res) {",
  ...
  "              res({ pcm_b64: floatToPcmB64(buf.getChannelData(0)), rate: buf.sampleRate || 16000, pcm_dtype: 'int16' });",
  "              ctx.close && ctx.close();",
  "            }, function () { res(null); });",
  "          };",
  ...
  "        });",
  "        sampleVideoFrames(file).then(function (frames) {",
  "          return audioP.then(function (audio) {",
  "            var metrics = metricsFromFrames(frames);",
  "            if (!metrics && !audio) {",
  "              reject(new Error('No audio or frames could be measured. Nothing was scored.'));",
  "              return;",
  "            }",
  "            var body = { filename: file.name, format: 'video', client_decode: 'browser' };",
  "            if (metrics) { body.visual = metrics.visual; body.video = metrics.video; }",
  "            if (audio) { body.pcm_b64 = audio.pcm_b64; body.rate = audio.rate; body.pcm_dtype = audio.pcm_dtype; }",
  "            resolve(body);",
  "          });",
  "        }).catch(reject);",
  "        return;",
  ...
  "            resolve({ pcm_b64: floatToPcmB64(ch), rate: rate, pcm_dtype: 'int16', filename: file.name, format: 'audio', client_decode: 'browser' });",
  "            ctx.close && ctx.close();",
  "          }, function () { reject(new Error('Could not decode that audio. Try WAV, or use vibelock detect locally.')); });",
  "        };",
  "        reader.onerror = function () { reject(new Error('Could not read that file.')); };",
  "        reader.readAsArrayBuffer(file);",
  "        return;",
  "      }",
  "      reject(new Error('Use WAV, MP3, MP4, WebM, or a still, or fill the feature fields. This page does not invent a score for a format the browser cannot decode.'));",
  "    });",
  "  }",
  "",
  "  function fail(msg) {",
  ...
  "    setText($('channels'), chans.length ? chans.map(function (c) {",
  "      return c.name + ' ' + c.status + (c.evidence && c.evidence !== 'none' ? ' (' + c.evidence + ')' : '');",
  "    }).join(' · ') : '—');",
  "    setText($('plain'), plain);",
  ...
  "    try {",
  "      if (!body.features && !body.visual && !body.video && !body.av && !body.pcm_b64 && !body.pitch && !body.linguistics && !body.vibration) {",
...
<div class="wrap">
  ...
  <section class="card" id="workspace">
    <h2>Analyze</h2>
    <p class="help">Run VibeLock in this browser. Paste feature notes, fill the same fields as <code>POST /v1/analyze</code>, or upload WAV, MP3, MP4, WebM, or a still. The browser decodes the file. This server does not. Results cite physics, linguistics, vibration, and related channels. A channel with no evidence stays insufficient. The score is a risk index, not an accuracy rate. Not a live mic.</p>
    <form id="analyze-form">
      ...
      <label for="media">Upload audio or video (WAV, MP3, MP4, M4A, WebM, MOV) or a still. The browser extracts measurements. The server does not decode the container.</label>
      <input id="media" type="file" accept="audio/*,video/*,image/*,.wav,.mp3,.mp4,.m4a,.m4v,.webm,.mov,.mkv,.ogg,.avi,.png,.jpg,.jpeg,.ppm">
      <details>
    ...
    <div class="answer" id="result" hidden>
      ...
      <div class="signals" id="channels"></div>
